...
std::unique_ptr<SingletonDebugFont> SingletonDebugFont::m_SingletonDebugFont = nullptr;

SingletonDebugFont* SingletonDebugFont::GetInstance()
{
	//すでに生成されてるかチェック
	if (m_SingletonDebugFont == nullptr)
	{
		//描画のインスタンスを生成する
		m_SingletonDebugFont = std::make_unique<SingletonDebugFont>();
	}
	//描画のインスタンスを返す
	return m_SingletonDebugFont.get();
}

// コンストラクタ
SingletonDebugFont::SingletonDebugFont()
	: m_fontHeight{}
{
}
...
// 描画する文字列を登録する関数
void SingletonDebugFont::AddString(const wchar_t * string, DirectX::SimpleMath::Vector2 pos, FXMVECTOR color, float scale)
{
	String str;
...
void SingletonDebugFont::Initialize(ID3D11Device* device, ID3D11DeviceContext* context, wchar_t const* fileName)
{
...
// 描画関数
void SingletonDebugFont::Render(DirectX::CommonStates* states)
{
...
// コンストラクタ
SingletonDebugFont3D::SingletonDebugFont3D(ID3D11Device* device, ID3D11DeviceContext* context, wchar_t const* fileName)
	: SingletonDebugFont()
{	
	SingletonDebugFont::Initialize(device, context, fileName);
	// エフェクトを作成
...
// デストラクタ
::SingletonDebugFont3D::~SingletonDebugFont3D()
{
	m_inputLayout.Reset();
	m_effect.reset();
}
...
// 描画する文字列を登録する関数（3D版）
void SingletonDebugFont3D::AddString(
...
// 描画関数（3D版）
void SingletonDebugFont3D::Render(
