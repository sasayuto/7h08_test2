...
/// インプットレイアウト
/// </summary>
const std::vector<D3D11_INPUT_ELEMENT_DESC> tito::GaugeManager::INPUT_LAYOUT =
{
...
/// コンストラクタ
/// </summary>
tito::GaugeManager::GaugeManager()
	:m_windowHeight(0)
	,m_windowWidth(0)
	,m_textureHeight(0)
	,m_textureWidth(0)
...
void tito::GaugeManager::LoadTexture(const wchar_t* path)
{
...
void tito::GaugeManager::SetScale(DirectX::SimpleMath::Vector2 scale)
{
	m_scale = scale;
}
void tito::GaugeManager::SetPosition(DirectX::SimpleMath::Vector2 position)
{
	m_position = position;
}
/// <summary>
/// Shader作成部分だけ分離した関数
/// </summary>
void tito::GaugeManager::CreateShader()
{
	auto device = SingletonDeviceResources::GetInstance()->GetDeviceResources()->GetD3DDevice();

	// コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shaders/GaugeVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shaders/GaugeGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/GaugePS.cso");
...
void tito::GaugeManager::Render()
{
	...
	VertexPositionColorTexture vertex[1] = {
		...
		, SimpleMath::Vector2(m_remderRatio,m_transparency))
	};

	//シェーダーに渡す追加のバッファを作成する。(ConstBuffer）
	ConstBuffer cbuff;
	cbuff.windowSize = SimpleMath::Vector4(static_cast<float>(m_windowWidth), static_cast<float>(m_windowHeight), static_cast<float>(m_positionAnchor), 1);
