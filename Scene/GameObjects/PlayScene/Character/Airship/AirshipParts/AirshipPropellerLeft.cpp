...
const float AirshipPropellerLeft::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float AirshipPropellerLeft::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float AirshipPropellerLeft::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float AirshipPropellerLeft::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)

const float PROPELLER_ACCELERATION(60.0f / 60);//プロペラの回転の加速度(°/s）
const float PROPELLER_COOL_TIME(0.95f);	//空気抵抗


AirshipPropellerLeft::AirshipPropellerLeft(IAirshipComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
...
AirshipPropellerLeft::~AirshipPropellerLeft()
{

}

void AirshipPropellerLeft::Initialize()
{	
	//360度ランダムな方向を取得
...
void AirshipPropellerLeft::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{	
	//加速度に空気抵抗をかける
...
void AirshipPropellerLeft::Render()
{
...
void AirshipPropellerLeft::Explosion()
{
...
void AirshipPropellerLeft::Finalize()
{
}

void AirshipPropellerLeft::Speed()
{
	//加速度計算
