...
const float AirshipElevatorRight::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float AirshipElevatorRight::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float AirshipElevatorRight::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float AirshipElevatorRight::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)


AirshipElevatorRight::AirshipElevatorRight(IAirshipComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
	AirshipComponent(parent, position, angle)
{

	//モデルをセット
	SetPartModel(Resources::GetInstance()->GetAirshipElevatorRightModel());
}

AirshipElevatorRight::~AirshipElevatorRight()
{

}

void AirshipElevatorRight::Initialize()
{	
	//360度ランダムな方向を取得
...
void AirshipElevatorRight::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{	
	//モデル位置調整
...
void AirshipElevatorRight::Render()
{
...
void AirshipElevatorRight::Explosion()
{
...
void AirshipElevatorRight::Finalize()
{
}

void AirshipElevatorRight::Speed()
{
	// パーツを更新する
	for (auto& playerPart : GetParts())
	{
