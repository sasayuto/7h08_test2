...
const float AirshipRoom_02::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float AirshipRoom_02::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float AirshipRoom_02::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float AirshipRoom_02::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)


AirshipRoom_02::AirshipRoom_02(IAirshipComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
	AirshipComponent(parent, position, angle)
{
	AddPart(AirshipPartFactory::CreateAirshipPropellerMiddle(parent,position,angle));
	//モデルをセット
	SetPartModel(Resources::GetInstance()->GetAirshipRoom_02Model());
}

AirshipRoom_02::~AirshipRoom_02()
{

}

void AirshipRoom_02::Initialize()
{	
	//360度ランダムな方向を取得
...
void AirshipRoom_02::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{	
	//モデル位置調整
	SetPosition(DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3(0.2f, -5.87f, -28.0f), angle));
...
void AirshipRoom_02::Finalize()
{
}

void AirshipRoom_02::Speed()
{
	// パーツを更新する
	for (auto& playerPart : GetParts())
	{
