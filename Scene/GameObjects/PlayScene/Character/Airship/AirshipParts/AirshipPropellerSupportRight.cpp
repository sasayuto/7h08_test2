...
const float AirshipPropellerSupportRight::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float AirshipPropellerSupportRight::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float AirshipPropellerSupportRight::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float AirshipPropellerSupportRight::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)


AirshipPropellerSupportRight::AirshipPropellerSupportRight(IAirshipComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
	AirshipComponent(parent, position, angle)
{
	AddPart(AirshipPartFactory::CreateAirshipPropellerRight(parent,position,angle));
	//モデルをセット
	SetPartModel(Resources::GetInstance()->GetAirshipPropellerSupportRightModel());
}

AirshipPropellerSupportRight::~AirshipPropellerSupportRight()
{

}

void AirshipPropellerSupportRight::Initialize()
{	
	//360度ランダムな方向を取得
...
void AirshipPropellerSupportRight::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{	
	//モデル位置調整
...
void AirshipPropellerSupportRight::Render()
{
...
void AirshipPropellerSupportRight::Explosion()
{
...
void AirshipPropellerSupportRight::Finalize()
{
}

void AirshipPropellerSupportRight::Speed()
{
	// パーツを更新する
	for (auto& playerPart : GetParts())
	{
