...
const float RightTire::FALL_SPEED_ACCELERATION(0.11f / 60);	//落下の加速度			(m/s)
const float RightTire::EXPLOSIVE_ROTATE(120.0f / 60);	//モデルの回転の速さ(°/s）
const float RightTire::EXPLOSIVE_RANDOM_POWER(1.4f / 60);	//爆発の横のパワー		(m/s)
const float RightTire::EXPLOSIVE_POWER(5.0f / 60);			//爆発の横のパワー		(m/s)

RightTire::RightTire(IPlayerComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
...
void RightTire::Initialize(const DirectX::SimpleMath::Vector3& reflectionVector)
{
...
void RightTire::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{
	//モデル位置調整
...
void RightTire::Finalize()
{
}

void RightTire::Speed()
{
}
