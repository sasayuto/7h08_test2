...
const float Wing::FALL_SPEED_ACCELERATION(0.11f / 60);	//落下の加速度			(m/s)
const float Wing::EXPLOSIVE_ROTATE(120.0f / 60);	//モデルの回転の速さ(°/s）
const float Wing::EXPLOSIVE_RANDOM_POWER(1.4f / 60);	//爆発の横のパワー		(m/s)
const float Wing::EXPLOSIVE_POWER(5.0f / 60);			//爆発の横のパワー		(m/s)

Wing::Wing(IPlayerComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
	PlayerComponent(parent,position,angle)
{
	//モデルをセット
	SetPartModel(Resources::GetInstance()->GetWingModel());
...
void Wing::Initialize(const DirectX::SimpleMath::Vector3& reflectionVector)
{
...
void Wing::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{
	//モデル位置調整
	SetPosition(SimpleMath::Vector3::Transform(SimpleMath::Vector3(0.0f, 0.18f, -0.12f), angle));
