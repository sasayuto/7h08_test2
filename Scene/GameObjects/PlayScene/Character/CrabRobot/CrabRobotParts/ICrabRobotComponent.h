...
/// </summary>
interface ICrabRobotComponent
{
	// 親のアドレスのセッター、ゲッター
	virtual ICrabRobotComponent* GetParent() = 0;

	//子のパーツの配列のゲッターセッター
	virtual std::vector<std::unique_ptr<ICrabRobotComponent>>& GetParts() = 0;
	virtual void AddPart(std::unique_ptr<ICrabRobotComponent> part) = 0; 
	...
	/// デストラクタ
	/// </summary>
	virtual ~ICrabRobotComponent() = default;
