...
const float CrabRobotHead::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float CrabRobotHead::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float CrabRobotHead::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float CrabRobotHead::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)
const int CrabRobotHead::MAX_HP(20000);	//HP


CrabRobotHead::CrabRobotHead(ICrabRobotComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle):
	CrabRobotComponent(
		parent,
		position + DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3(0, 40.0f, 0), angle),
		angle
	)
{
	//モデルをセット
	SetPartModel(Resources::GetInstance()->GetCrabRobotHead());
...
CrabRobotHead::~CrabRobotHead()
{

}

void CrabRobotHead::Initialize()
{
	//360度ランダムな方向を取得
...
void CrabRobotHead::Update(const float& elapsedTime, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{

	//親が動かないから更新しない//
	//親の位置と合わせる
	SetPosition(position + DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3(0, 40.0f, 0), angle));
	//親の回転と合わせる
	SetRotate(angle);
	//当たり判定の更新
...
void CrabRobotHead::Explosion(const float& elapsedTime)
{
	//アニメーションを超えたら更新しない
