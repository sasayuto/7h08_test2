...
//ゲームの初期化を行う関数
void BigBrickWallManager::Intialize()
{
	...
	if (obj.is_open()) {
		...
		if (Json::parseFromStream(reader, obj, &root, &errors))
		{
			...
			if (root.isMember("objects") && root["objects"].isArray())
			{
				...
				// "house" 配列を取得します。
				const Json::Value& bigBrickWallArray = root["objects"][0]["BigBrickWall"];

				// "house" 配列内の各要素に対してループを実行します。
				for (unsigned i = 0; i < bigBrickWallArray.size(); i++)
				{
					// 新しいオブジェクトを作成し、位置と回転を設定します。
					std::unique_ptr<Objects> bigBrickWall = std::make_unique<BigBrickWall>();
					bigBrickWall->SetPosition(SimpleMath::Vector3(
						bigBrickWallArray[i]["position"][0].asFloat(),
						bigBrickWallArray[i]["position"][1].asFloat(),
						bigBrickWallArray[i]["position"][2].asFloat()
					));
					bigBrickWall->SetRotateJson(SimpleMath::Vector3(
						bigBrickWallArray[i]["rotate"][0].asFloat(),
						bigBrickWallArray[i]["rotate"][1].asFloat(),
						bigBrickWallArray[i]["rotate"][2].asFloat()
					));

					// リストにオブジェクトを追加します。
					m_objectsList.push_back(std::move(bigBrickWall));
				}
			}
			else {
...
void BigBrickWallManager::CollJson(const Json::Value& ary, Objects* objects)
{
	// 当たり判定を確認したいときに使えるよ
...
//ゲームの更新を行う関数
void BigBrickWallManager::Update()
{
	//m_objectsList.clear();
	//Intialize();
...
//ゲームの更新処理を行う関数
void BigBrickWallManager::Render()
{
	//球の描画
	for (int i = 0; i < m_objectsList.size(); i++)
	{
...
void BigBrickWallManager::RenderShadowTex()
{
	//球の描画
	for (int i = 0; i < m_objectsList.size(); i++)
	{
...
void BigBrickWallManager::Finalize()
{
	//後処理
	for (int i = 0; i < m_objectsList.size(); i++)
	{
...
void BigBrickWallManager::CheckHitCharacter(Player* player)
{
	for (int i = 0; i < m_objectsList.size(); i++)
	{
		//大まかなプレイヤーとの当たり判定
...
void BigBrickWallManager::CheckHitEnemy(Enemy* enemy)
{
	for (int i = 0; i < m_objectsList.size(); i++)
	{
		//大まかなプレイヤーとの当たり判定
...
void BigBrickWallManager::CheckHitAirship(Airship* airship)
{
	for (int i = 0; i < m_objectsList.size(); i++)
	{
		//大まかなプレイヤーとの当たり判定
