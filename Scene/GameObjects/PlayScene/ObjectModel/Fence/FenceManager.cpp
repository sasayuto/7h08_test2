...
FenceManager::FenceManager():
	MoveObjectsManager()
{
	std::ifstream obj("Resources/json/TwonScene/Json_TwonFence.Json");
	...
	if (obj.is_open())
	{
		...
		{
			const Json::Value& wallsArray = root["Fence"];
			for (const Json::Value& wallDate : wallsArray)
			{
				//floatからVector3に変換
				std::unique_ptr<MoveObjects> windmill = std::make_unique<Fence>();
