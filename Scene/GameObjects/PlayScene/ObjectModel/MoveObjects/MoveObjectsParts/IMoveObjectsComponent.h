...
/// </summary>
interface IMoveObjectsComponent
{
	// 親のアドレスのセッター、ゲッター
	virtual IMoveObjectsComponent* GetParent() = 0;
	virtual void SetParent(IMoveObjectsComponent* parent) = 0;

	//
	virtual std::vector<std::unique_ptr<IMoveObjectsComponent>>& GetParts() = 0;
	virtual void AddPart(std::unique_ptr<IMoveObjectsComponent> part) = 0;
	...
	/// デストラクタ
	/// </summary>
	virtual ~IMoveObjectsComponent() = default;
	...
	virtual IMoveObjectsComponent* Collision(Enemy* enemy) = 0;
	virtual IMoveObjectsComponent* Collision(Airship* airship) = 0;
