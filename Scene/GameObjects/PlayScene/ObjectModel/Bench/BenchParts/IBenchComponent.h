...
/// </summary>
interface IBenchComponent
{
	// 親のアドレスのセッター、ゲッター
	virtual IBenchComponent* GetParent() = 0;
	virtual void SetParent(IBenchComponent* parent) = 0;

	//
	virtual std::vector<std::unique_ptr<IBenchComponent>>& GetParts() = 0;
	virtual void AddPart(std::unique_ptr<IBenchComponent> part) = 0;
	...
	/// デストラクタ
	/// </summary>
	virtual ~IBenchComponent() = default;
	...
	virtual IBenchComponent* Collision(Enemy* enemy) = 0;
	virtual IBenchComponent* Collision(Airship* airship) = 0;
