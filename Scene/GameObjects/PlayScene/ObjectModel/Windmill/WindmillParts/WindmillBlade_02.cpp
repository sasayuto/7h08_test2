...
const float WindmillBlade_02::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float WindmillBlade_02::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float WindmillBlade_02::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float WindmillBlade_02::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)

const float PROPELLER_ACCELERATION(2.0f / 60);//プロペラの回転の加速度(°/s）
const float PROPELLER_COOL_TIME(0.95f);	//空気抵抗

WindmillBlade_02::WindmillBlade_02(IWindmillComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
	WindmillComponent(
		parent,
...
WindmillBlade_02::~WindmillBlade_02()
{

}

void WindmillBlade_02::Initialize()
{	
	//当たり判定の初期化
...
void WindmillBlade_02::Update(const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{	
	//加速度計算
	...
	SetRotate(DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(
...
void WindmillBlade_02::Finalize()
{
}
