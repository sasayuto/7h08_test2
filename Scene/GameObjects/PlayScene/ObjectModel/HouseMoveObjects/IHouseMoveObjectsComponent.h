...
/// </summary>
interface IHouseMoveObjectsComponent
{
	// 親のアドレスのセッター、ゲッター
	virtual IHouseMoveObjectsComponent* GetParent() = 0;
	virtual void SetParent(IHouseMoveObjectsComponent* parent) = 0;

	//
	virtual std::vector<std::unique_ptr<IHouseMoveObjectsComponent>>& GetParts() = 0;
	virtual void AddPart(std::unique_ptr<IHouseMoveObjectsComponent> part) = 0;
	...
	//回転のゲッターセッター
	virtual DirectX::SimpleMath::Quaternion GetInitialRotate() const = 0;
	virtual void SetInitialRotate(const DirectX::SimpleMath::Quaternion& rotate) = 0;
	...
	/// デストラクタ
	/// </summary>
	virtual ~IHouseMoveObjectsComponent() = default;
	...
	virtual IHouseMoveObjectsComponent* Collision(Enemy* enemy) = 0;
	virtual IHouseMoveObjectsComponent* Collision(Airship* airship) = 0;
