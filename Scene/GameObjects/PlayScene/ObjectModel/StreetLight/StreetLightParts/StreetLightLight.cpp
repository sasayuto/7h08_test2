...
const float StreetLightLight::FALL_SPEED_ACCELERATION	(0.12f / 60);	//落下の加速度			(m/s)
const float StreetLightLight::EXPLOSIVE_ROTATE			(120.0f / 60);	//モデルの回転の速さ	(°/s）
const float StreetLightLight::EXPLOSIVE_RANDOM_POWER(1.4f / 60);		//爆発の横のパワー		(m/s)
const float StreetLightLight::EXPLOSIVE_POWER(2.0f / 60);		//爆発の横のパワー		(m/s)

StreetLightLight::StreetLightLight(IStreetLightComponent* parent, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle) :
	StreetLightComponent(
		parent,
		position + DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3(0.6f, -0.96f, 0), angle),
		angle
	)
{
	//モデルをセット
	SetPartModel(Resources::GetInstance()->GetStreetLightLightModel());
}

StreetLightLight::~StreetLightLight()
{

}

void StreetLightLight::Initialize()
{
	//360度ランダムな方向を取得
...
void StreetLightLight::Update(const float& elapsedTime, const DirectX::SimpleMath::Vector3& position, const DirectX::SimpleMath::Quaternion& angle)
{
...
void StreetLightLight::Explosion(const float& elapsedTime)
{
	//アニメーションを超えたら更新しない
...
void StreetLightLight::Finalize()
{
}
