...
/// </summary>
class StreetLightComponent :public IStreetLightComponent
{
public:
	// 親のゲッターセッター
	IStreetLightComponent* GetParent()override { return m_pParent; }
	void SetParent(IStreetLightComponent* parent) override { m_pParent = parent; }
	...
	//子のパーツのゲッター
	std::vector<std::unique_ptr<IStreetLightComponent>>& GetParts()override { return m_PlayerParts; }

	//子のパーツを生成
	void AddPart(std::unique_ptr<IStreetLightComponent> part) override{ m_PlayerParts.push_back(std::move(part)); }
...
public:
	...
	/// デストラクタ
	/// </summary>
	virtual ~StreetLightComponent() override {}
	...
	virtual bool CollisionPlayer(Player* player)override {
		{
			...
			for (auto& playerParts : player->GetParts())
			{
				...
				if (playerParts->CollisionStreetLightParts(this))
				{
					//書けば当たったパーツの後のパーツの判定を行ってくれる
	...
	virtual IStreetLightComponent* Collision(Enemy* enemy)override {
		if (GetCollider()->CheckHitSphere(enemy->GetCollider()))return this;
		return nullptr;
	};
	virtual IStreetLightComponent* Collision(Airship* airship)override {
...
private:
	...
	//親のポインタ
	IStreetLightComponent* m_pParent;

	//パーツの配列のポインタ
	std::vector<std::unique_ptr<IStreetLightComponent>> m_PlayerParts;
};
